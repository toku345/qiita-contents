---
title: ClojureScript内からnpmモジュールを呼び出す（npm-deps）
tags:
  - ClojureScript
  - npm
private: false
updated_at: "2017-12-09T12:24:36+09:00"
...
project.clj にコンパイラ・オプション `:npm-deps` と `:install true` を追加。
...
ちょっとググってみたところ、[CLJSJS](https://cljsjs.github.io/)を使うといいよ〜とか、そこにない場合は[自力で externs を書く必要があるよ](https://clojurescript.org/reference/packaging-foreign-deps)とか、いろいろ見つかるのですが、どれもうまく行かなかったり設定に戸惑ったりすること１週間...[^2]

[^2]: 最新の[AWS SDK for javascript](https://github.com/aws/aws-sdk-js)と[Alexa Skill Kit](https://github.com/amzn/alexa-skills-kit-js)を使いたかったのです
...
project.clj に `:npm-deps {:aws-sdk "^2.167.0"}` と `:install true`を追加。
...
依存関係インストール + cljs コード → JS コードへのトランスパイル + REPL を開くために `lein figwheel`を実行。
